  ...
  if(!is.null(data$waa_pointer_fleets)){
    ...
    if(!is.null(waa_info$waa_pointer_fleets)) {
      if(any(!(waa_info$waa_pointer_fleets %in% 1:dim(data$waa)[1]))){
        stop("some basic_info$waa_pointer_fleets are outside the number of waa matrices.\n")
      }
      if(length(waa_info$waa_pointer_fleets) != data$n_fleets){
        stop("length of basic_info$waa_pointer_fleets is not equal to the number of stocks.\n")
      }
