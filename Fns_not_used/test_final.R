...
log_catch_fleets_F_multi <- function(log_F, NAA, log_M, mu, sel, fracyr_season, fleet_regions, fleet_seasons, can_move, mig_type, waacatch) {
...
# for (year in 1:30) {
#   cat("In Year ", year <- year)
#   Catch = om$input$data$agg_catch[year,]
#   Fsolve = get_F_from_Catch(Catch, year, method = "nlminb", om)
#   Ftrue = max(exp(rep$log_FAA_tot[year,]))
#   print(paste(Fsolve - Ftrue))
# }
year = 30
Catch = om$input$data$agg_catch[year,]

get_F_from_Catch(Catch, year, method = "nlminb", om)
...
for (year in 1:30) {
  ...
  Fsolve = get_F_from_Catch(Catch, year, method = "nlminb", om)
  Fsolve = Fsolve*sel[,8]
  Ftrue = rep$FAA[,year,8]
  num = sum(round(Fsolve - Ftrue,3))
  res[year] = num
  cat("\n",paste(num),"\n")
}
plot(res,type = "l")
...
for (year in 1:30) {
  ...
  Catch = om$input$data$agg_catch[year,]
  Fsolve = get_F_from_Catch(Catch, year, method = "nlminb", om)
  Fsolve = Fsolve*om$rep$sel_static[,8]
  Ftrue = rep$FAA[,year,8]
  num = sum(round(Fsolve - Ftrue,3))
  res[year] = num
  cat("\n",paste(num),"\n")
