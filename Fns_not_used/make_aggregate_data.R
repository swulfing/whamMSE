...
make_aggregate_data <- function(om, em_info, aggregate_catch_info, aggregate_index_info, ind_em) {
  ...
  aggregated_fleet_waa = list()
  for (f in unique(valid_fleets)) {
      aggregated_fleet_waa[[f]] <- as.matrix(em_info$par_inputs$user_waa$waa[which(fleet_pointer == f),,])
      if (ncol(aggregated_fleet_waa[[f]]) > 1) {
        aggregated_fleet_waa[[f]] <- colSums(aggregated_fleet_waa[[f]] * fleet_weights[[f]])
      } else {
        aggregated_fleet_waa[[f]] <- sum(aggregated_fleet_waa[[f]] * fleet_weights[[f]])
  ...
  for (f in unique(valid_indices)) {
    ind <- which(index_pointer == f)  # Which indices to aggregate
    n_years <- length(ind_em)
    agg_waa <- matrix(0, nrow = n_ages, ncol = n_years)  # [age, year]
    
    for (y in 1:length(ind_em)) {
      
      if (aggregate_index_info$use_catch_weighted_waa) {
        year_values <- data$agg_indices[y, index_group]
      } else {
        year_values <- rep(1, length(index_group))  # Equal weights
      }
      
      # Normalize weights
      weight_sum <- sum(year_values, na.rm = TRUE)
      weights <- if (weight_sum > 0) year_values / weight_sum else rep(1 / length(index_group), length(index_group))
      
      # Aggregate WAA for this year
      for (i in seq_along(index_group)) {
        idx <- index_group[i]
        waa_matrix <- em_info$par_inputs$user_waa$waa[idx, , ind_em[y]]  # [age]
        agg_waa[, y] <- agg_waa[, y] + weights[i] * waa_matrix
      }
    }
    
    aggregated_index_waa[[g]] <- agg_waa  # [age, year]
  }
