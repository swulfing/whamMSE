...
for (s in 1:2){
  for(a in 1:8){
    print(annual_Ps[s,a,,]-nofit_move$rep$annual_Ps[s,33,a,,]) # stock x age x 7 x 7
  }
}
...
log_catch_fleets_F_multi <- function(log_F, NAA, log_M, mu, sel, fracyr_seasons, fleet_regions, 
  ...
  for (s in 1:n_stocks) {
    for (a in 1:n_ages) {
      P_ya <- I
      for (t in 1:n_seasons) {
        P_ya <- P_ya %*% get_P_t(a, s, t, fleet_regions, fleet_seasons, can_move, mig_type, fracyr_season[t], FAA_T, log_M, mu)
      }
      for (r in 1:n_regions) {
        for (f in 1:n_fleets) {
  ...
  # annual_Ps <- get_annual_Ps(fleet_regions, fleet_seasons, can_move, mig_type, fracyr_seasons,
  #                            FAA_T, log_M, mu)
  # 
  # for (s in 1:n_stocks) {
  #   for (a in 1:n_ages) {
  ...
  # }
  # 
  # pred_stock_catch <- array(0, dim = c(n_fleets,n_stocks, n_ages))
  # 
  # for (f in 1:n_fleets) {
  #   for (a in 1:n_ages) {
  #     for (s in 1:n_stocks) {
  #       for (r in 1:n_regions) {
  #         pred_stock_catch[f,s,a] = NAA[s,r,a] * annual_Ps[s,a,r,n_regions+f] * waacatch[f,a]
  #       }
  #     }
  #   }
  # }
  # 
  # pred_catch = rep(0,n_fleets)
  # 
  # for (f in 1:n_fleets) {
  #   for (a in 1:n_ages) {
  #     for (s in 1:n_stocks) {
  #       pred_catch[f] = pred_catch[f] + pred_stock_catch[f,s,a] 
  #     }
  #   }
  # }
  # 
...
#     P_ya <- I
#     for (t in 1:n_seasons) {
#       P_ya <- P_ya %*% get_P_t(a, s, t, fleet_regions, fleet_seasons, can_move, mig_type, fracyr_season[t], FAA, log_M, mu)
#     }
#     for (r in 1:n_regions) {
#       for (f in 1:n_fleets) {
#         catch_stock_fleet[s, f] <- catch_stock_fleet[s, f] + NAA[s, r, a] * P_ya[r, n_regions + f] * waacatch[f, a]
...
# }
# 
# colSums(catch_stock_fleet)
# 
# 
# om$rep$pred_catch[year,]
